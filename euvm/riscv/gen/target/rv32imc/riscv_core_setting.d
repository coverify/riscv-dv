...
riscv_instr_group_t[] supported_isa = [riscv_instr_group_t.RV32I, riscv_instr_group_t.RV32M, riscv_instr_group_t.RV32C];
...
enum privileged_reg_t[] implemented_csr = [
    ...
    enum privileged_reg_t.MVENDORID,  // Vendor ID
    enum privileged_reg_t.MARCHID,    // Architecture ID
    enum privileged_reg_t.MIMPID,     // Implementation ID
    enum privileged_reg_t.MHARTID,    // Hardware thread ID
    enum privileged_reg_t.MSTATUS,    // Machine status
    enum privileged_reg_t.MISA,       // ISA and extensions
    enum privileged_reg_t.MIE,        // Machine interrupt-enable register
    enum privileged_reg_t.MTVEC,      // Machine trap-handler base address
    enum privileged_reg_t.MCOUNTEREN, // Machine counter enable
    enum privileged_reg_t.MSCRATCH,   // Scratch register for machine trap handlers
    enum privileged_reg_t.MEPC,       // Machine exception program counter
    enum privileged_reg_t.MCAUSE,     // Machine trap cause
    enum privileged_reg_t.MTVAL,      // Machine bad address or instruction
    enum privileged_reg_t.MIP         // Machine interrupt pending
];
